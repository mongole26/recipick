...
| # | 작업 | 상태 |
|---|------|------|
| 5.1 | 홈 화면 — 재료 입력이 홈 역할 (MVP) | ✅ |
| 5.2 | 추천 결과 화면 — 레시피 카드 3개 (매칭률, 부족 재료, AI 이유) | ✅ |
| 5.3 | 레시피 상세 화면 — 재료 목록 (보유/부족 표시), 조리 순서 | ✅ |
| 5.4 | 로딩 상태 — 버튼 로딩 + 상세화면 스피너 | ✅ |
| 5.5 | 에러 상태 — 빈 결과 EmptyState, 상세 실패 재시도 | ✅ |
| 5.6 | 온보딩 카드 3장 (최초 실행 시) | 🔲 (P1에서 진행) |
...
| # | 작업 | 상태 |
|---|------|------|
| 6.1 | 모델 단위 테스트 (파싱, 매칭률, 포맷) | ✅ (13개 통과) |
| 6.2 | 재료 데이터 테스트 (검색, 단위) | ✅ (8개 통과) |
| 6.3 | Edge Function 통합 테스트 (curl) | ✅ (정상/에러 확인) |
| 6.4 | 엣지 케이스 처리 (재료 0개, 매칭 0건, 네트워크 오류) | ✅ |
| 6.5 | 성능 확인 (추천 2.7초, 3초 이내 달성) | ✅ |
| 6.6 | 코드 정리 + 린트 패스 (에러 0개) | ✅ |
