...
| **임베딩** | Google Gemini `gemini-embedding-001` (768차원) | 무료, 한국어 성능 양호 |
| **LLM** | Google Gemini Flash | 무료, 한국어 성능 양호, 빠른 응답 |
...
|-------|------|------|
| recipe_id | uuid | FK → recipes |
| embedding | vector(768) | 레시피 임베딩 벡터 (Gemini embedding-001) |
...
② 벡터 유사도 검색 (pgvector)
   - 사용자 재료 목록을 텍스트로 조합
   - Gemini Embedding API로 벡터화 (768차원)
...
| Gemini Flash (Claude 대신) | 무료, 충분한 한국어 성능 |
| 비회원 허용 | 진입 장벽 낮춤, 로컬 → 클라우드 마이그레이션 지원 |
| 임베딩은 Gemini | gemini-embedding-001 무료 + 한국어 성능 양호 |
